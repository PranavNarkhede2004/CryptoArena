...
// Alternative exchanges for fallback
const ALTERNATIVE_APIS = [
  {
    ...
    parseResponse: (data, symbol) => {
      const mappedSymbol = symbolMap[symbol] || symbol.toLowerCase();
      const ticker = data.result[mappedSymbol];
      if (!ticker) return null;
      
      return {
        s: symbol,
        c: ticker.c[0], // last price
        P: ((parseFloat(ticker.c[0]) - parseFloat(ticker.o)) / parseFloat(ticker.o) * 100).toFixed(2), // change %
        q: ticker.v[1], // quote volume
        h: ticker.h[0], // high
        l: ticker.l[0], // low
        o: ticker.o // open
      };
  ...
  },
  {
    ...
    parseResponse: (data, symbol) => {
      ...
      const ticker = data.find(t => t.id === mappedSymbol);
      if (!ticker) return null;
      
      const changePercent = ((parseFloat(ticker.price) - parseFloat(ticker.open_24h)) / parseFloat(ticker.open_24h) * 100).toFixed(2);
      return {
        s: symbol,
        c: ticker.price,
        P: changePercent,
        q: ticker.volume_24h * parseFloat(ticker.price),
        h: ticker.high_24h,
        l: ticker.low_24h,
        o: ticker.open_24h
      };
...
const fetchFromAlternativeExchanges = async () => {
  console.log('Trying alternative exchanges...');
  const symbols = TRACKED_BASES.map(base => `${base}USDT`);
  
  for (const exchange of ALTERNATIVE_APIS) {
    try {
      console.log(`Trying ${exchange.name}...`);
      
      if (exchange.name === 'Kraken') {
        const response = await axios.get(exchange.baseUrl, {
          params: { pair: symbols.map(s => exchange.symbolMap[s]).join(',') },
          timeout: 5000
        });
        
        const tickers = symbols.map(symbol => 
          exchange.parseResponse(response.data, symbol)
        ).filter(Boolean);
        
        if (tickers.length > 0) {
          console.log(`Successfully fetched ${tickers.length} tickers from ${exchange.name}`);
          return tickers;
        }
      } else if (exchange.name === 'Coinbase') {
        const response = await axios.get(exchange.baseUrl, { timeout: 5000 });
      ...
      console.warn(`${exchange.name} failed:`, error.message);
      continue;
    }
  }
...
const startFallbackSimulation = () => {
  ...
  fallbackInterval = setInterval(async () => {
    ...
    try {
      // First try alternative exchanges
      const alternativeData = await fetchFromAlternativeExchanges();
      
      if (alternativeData) {
        const usdtToInr = fxState.rate || 94.0;
        const payload = [];
        
        alternativeData.forEach(ticker => {
          const base = ticker.s.replace('USDT', '');
          if (!TRACKED_BASES.includes(base)) return;
          
          const payload = buildMarketPayload([ticker]);
          payload.push(...payload);
        });
        
        if (payload.length > 0 && ioRef) {
          console.log(`Broadcasting ${payload.length} prices from alternative exchange`);
        ...
        try {
          ...
          // If too many consecutive failures, switch to static data
          if (consecutiveFailures >= 5) {
            console.log('Too many consecutive failures, switching to static data simulation');
            useStaticData = true;
