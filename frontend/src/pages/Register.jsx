...
import { Eye, EyeOff, Mail, Lock, Zap, CheckCircle2, TrendingUp, XCircle, Loader2, User } from 'lucide-react';
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-[#080b11] text-[#E8EDF5] selection:bg-accent selection:text-background font-body overflow-hidden w-full">
      ...
      <div className="hidden md:flex md:w-[60%] relative flex-col justify-between p-12 bg-[#060810] overflow-hidden border-r border-[#151b23]">
        ...
        <div className="relative z-10 max-w-xl">
          ...
          >
            Start Building,<br />
            <span className="text-accent">Commander<span className="animate-cursor-blink inline-block w-4 h-1 bg-accent ml-1 align-baseline rounded-sm" /></span>
          </motion.h1>
          <motion.p 
            initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }}
            className="text-lg text-textSecondary mb-10"
          >
            Simulate strategies. Master the markets.
          </motion.p>
        ...
        >
          <Link to="/login" className="text-sm font-medium text-textSecondary hover:text-textPrimary transition-colors group">
            Already registered? <span className="text-accent group-hover:underline">Sign in here →</span>
          </Link>
        </motion.div>
      </div>

      {/* RIGHT PANEL: Registration Form Center Container */}
        ...
        >
          ...
          <div className="bg-[#0b0e14] border border-borderSubtle bg-gradient-to-b from-[#0e121a] to-[#080b11] rounded-2xl p-8 shadow-2xl">
            <div className="mb-8 text-center">
              <div className="text-xs font-mono font-bold text-accent tracking-widest uppercase mb-2">Register</div>
              <h2 className="text-[28px] font-heading font-bold leading-tight">Create Profile</h2>
              <p className="text-sm text-textMuted mt-1 font-medium">Set up your account to get started</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <div className="relative group">
                  <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                    <User size={18} className="text-textMuted group-focus-within:text-accent transition-colors" />
                  </div>
                  <input
                    type="text"
                    value={username}
              ...
              <div className="flex items-center justify-between pt-1 pb-4">
                <label className="flex items-center gap-2 cursor-pointer group">
                  ...
                  <span className="text-xs font-medium text-textMuted group-hover:text-textSecondary transition-colors">I agree to the Terms of Service</span>
                </label>
              </div>
              ...
              >
                {isSubmitting ? <><Loader2 size={18} className="animate-spin" /> CREATING PROFILE</> : 'CREATE PROFILE →'}
              </button>
            </form>
